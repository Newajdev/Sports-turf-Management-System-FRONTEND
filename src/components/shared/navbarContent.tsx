...
export default function NavbarContent({ userInfo }: navbarProps) {
  ...
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    ...
    const dashboard = pathname.startsWith("/dashoboard")
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <nav className="flex items-center justify-between">
          ...
          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center justify-between gap-10 w-[65%]">
                ...
                <div className="flex items-center gap-3 pl-2 border-l border-muted/50 h-8">
                  <div className="hidden sm:flex flex-col items-end">
                    ...
                    <span
                      className={cn(
                        !dashboard
                          ? "text-[10px] text-primary bg-primary/20 px-1 rounded-sm uppercase tracking-tight"
                          : "text-[10px] text-muted-foreground uppercase tracking-tight",
                        showActiveState
                          ? "text-foreground hover:text-primary"
                          : "text-white/90 hover:text-white hover:bg-white/10",
                      )}
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-background/95 backdrop-blur-2xl border-b border-white/10 p-8 animate-in slide-in-from-top-4 duration-500 rounded-b-3xl shadow-2xl">
            <ul className="flex flex-col gap-6 mb-8  items-end">
              {navLinks.map((link) => (
                <li key={link.name} className="w-full text-start ">
                  <Link
                    href={link.href}
                    className="text-xl font-black text-black hover:text-primary transition-all uppercase italic tracking-[0.2em]"
              ...
              <div className="flex flex-col gap-4">
                <Link
                  ...
                  className={cn(
                    ...
                    "w-full h-14 border-white/20 text-white font-bold italic",
                  )}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Log in
                ...
                <Link
                  ...
                  className={cn(
                    buttonVariants(),
                    "w-full h-14 bg-primary text-white font-black uppercase italic tracking-widest",
                  )}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Join the Pitch
